...
const ProductVariants = ({ colors, sizes, selectedColor, selectedSize, handleSelectColor, setSelectedSize }) => {
  return (
        ...
        <div className="mt-4">
          ...
          >
            {sizes.map((size, index) => (
              <div key={index} className="flex items-center space-x-2">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <RadioGroupItem value={size} id={`size-${size}`} className="peer sr-only" checked={selectedSize === size} />
                  <Label
                    htmlFor={`size-${size}`}
